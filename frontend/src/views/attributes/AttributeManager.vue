...
async function loadPreviewMa() {
  const hasAutoMa = config.value?.fields?.some((f) => f.autoGenerated && f.key === 'ma')
  if (!hasAutoMa || modalMode.value !== 'add' || !config.value?.api?.getMaTiepTheo) return
  form.value.ma = '...'
  try {
    const res = await config.value.api.getMaTiepTheo()
    form.value.ma = res.data?.ma || ''
  } catch {
    form.value.ma = ''
  }
}
...
function validateForm() {
  for (const field of config.value.fields) {
    if (field.autoGenerated && field.key === 'ma' && modalMode.value === 'add') continue
  ...
  if (modalMode.value === 'edit') {
    ...
    if (maField) {
      ...
      if (ma) {
        ...
        if (duplicated) {
          return `Mã "${form.value.ma}" đã tồn tại.`
        }
      }
    }
  }
