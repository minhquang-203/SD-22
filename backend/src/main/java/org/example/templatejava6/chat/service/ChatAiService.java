...
public class ChatAiService {
    ...
    private TinNhanChatAi generateGeminiResponse(
        ...
        if (apiKey == null) {
            log.warn("Gemini API key chưa cấu hình (GEMINI_API_KEY / application-local.properties) — fallback");
        ...
        try {
            ...
            // Model flash đời mới tính cả token "suy nghĩ" vào output — để thấp sẽ bị cắt cụt câu
            generationConfig.put("maxOutputTokens", 2048);

            String body = objectMapper.writeValueAsString(rootNode);
            String preferred = (geminiModel == null || geminiModel.isBlank()) ? "gemini-flash-latest" : geminiModel.trim();
            // Dự phòng model đời mới — các model cũ (2.0/1.5) đã hết quota hoặc bị gỡ với key mới
            List<String> modelsToTry = new ArrayList<>(List.of(preferred, "gemini-flash-latest", "gemini-flash-lite-latest", "gemini-2.5-flash-lite"));
            modelsToTry = new ArrayList<>(new LinkedHashSet<>(modelsToTry));
            ...
            // Lấy sản phẩm gợi ý từ tag đầu tiên hợp lệ, nhưng xóa TẤT CẢ tag khỏi text hiển thị
            SanPham spGoiY = null;
            Matcher matcher = PRODUCT_ID_PATTERN.matcher(aiText);
            while (matcher.find() && spGoiY == null) {
    ...
    private ChatResponseDto mapToDto(TinNhanChatAi entity, Map<Integer, SanPhamResponse> byId) {
        ...
        if (entity.getSanPhamGoiY() != null) {
            spRes = byId.get(entity.getSanPhamGoiY().getId());
        }
